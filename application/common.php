...
// CurlPOST数据提交-----------------------------------------
function mac_curl_post($url, $data, $heads = [], $cookie = '')
{
    $ch = @curl_init();
...
// CurlPOST数据提交-----------------------------------------
function mac_curl_get($url, $heads = [], $cookie = '')
{
    $ch = @curl_init();
    curl_setopt($ch, CURLOPT_USERAGENT,
        'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/45.0.2454.101 Safari/537.36');
...
{
    $url = base64_decode('aHR0cDovL2FwaS5tYWNjbXMucHJv').'/keyword/index?name='.rawurlencode($title).'&txt='.rawurlencode($title).rawurlencode(mac_substring(strip_tags($content),
            200));
...
function mac_get_aid($controller, $action = '')
{
    ...
    $arr = [
        'index' => 1, 'map' => 2, 'rss' => 3, 'gbook' => 4, 'comment' => 5, 'user' => 6, 'label' => 7, 'vod' => 10,
        'art'   => 20, 'topic' => 30, 'actor' => 80, 'role' => 90, 'plot' => 100, 'website' => 110
    ];
    $res = $arr[$controller];

    $arr = [
        'vod/type'     => 11, 'vod/show' => 12, 'vod/search' => 13, 'vod/detail' => 14, 'vod/play' => 15,
        'vod/down'     => 16, 'vod/role' => 17,
...
{
    $arr = [
        1 => lang_web('integral_recharge'), 2 => lang_web('registration_promotion'), 3 => lang_web('visit_promotion'),
        4 => lang_web('one_level_distribution'), 5 => lang_web('two_level_distribution'),
        6 => lang_web('three_level_distribution'), 7 => lang_web('points_upgrade'),
        8 => lang_web('integral_consumption'), 9 => lang_web('integral_withdrawal')
    ];
    return $arr[$data];
}
...
{
    $arr = [
        1 => 'integral_recharge', 2 => 'registration_promotion', 3 => 'visit_promotion', 4 => 'one_level_distribution',
        5 => 'two_level_distribution', 6 => 'three_level_distribution', 7 => 'points_upgrade',
        8 => 'integral_consumption', 9 => 'integral_withdrawal'
    ];
    return $arr[$data];
}
...
function mac_url_img($url)
{
    ...
    } elseif (!empty($GLOBALS['config']['upload']['img_key']) && preg_match('/'.$GLOBALS['config']['upload']['img_key'].'/',
            $url)) {
...
function mac_url($model, $param = [], $info = [])
{
    ...
    $replace_from = [
        '{id}', '{en}', '{page}', '{type_id}', '{type_en}', '{type_pid}', '{type_pen}', '{md5}', '{year}', '{month}',
        '{day}', '{sid}', '{nid}'
    ];
    $replace_to = [];
    ...
    switch ($model) {
        ...
        case 'website/show':
            switch ($config['rewrite']['type_id']) {
                case 1:
                    ...
                    break;
                case 2:
                    $id = mac_alphaID($info['type_id'], false, $config['rewrite']['encode_len'],
                        $config['rewrite']['encode_key']);
                    break;
                default:
        ...
        case 'vod/detail':
            ...
            if ($config['view']['vod_detail'] == 2) {
                ...
                switch ($config['rewrite']['vod_id']) {
                    case 1:
                        ...
                        break;
                    case 2:
                        $id = mac_alphaID($info['vod_id'], false, $config['rewrite']['encode_len'],
                            $config['rewrite']['encode_key']);
                        break;
                    default:
                        $id = $info['vod_id'];
            ...
            $replace_to = array_merge($replace_to,
        ...
        case 'vod/play':
            ...
            $replace_to = array_merge($replace_to, [
                date('Y', $info['vod_time']), date('m', $info['vod_time']), date('d', $info['vod_time']), $param['sid'],
                $param['nid']
            ]);
            break;
        case 'vod/down':
            ...
            if ($config['view']['art_detail'] == 2) {
                ...
                switch ($config['rewrite']['art_id']) {
                    case 1:
                        ...
                        break;
                    case 2:
                        $id = mac_alphaID($info['art_id'], false, $config['rewrite']['encode_len'],
                            $config['rewrite']['encode_key']);
                        break;
                    default:
                        $id = $info['art_id'];
            ...
            if ($config['view']['topic_detail'] == 2) {
                ...
                switch ($config['rewrite']['topic_id']) {
                    case 1:
                        ...
                        break;
                    case 2:
                        $id = mac_alphaID($info['topic_id'], false, $config['rewrite']['encode_len'],
                            $config['rewrite']['encode_key']);
                        break;
                    default:
                        $id = $info['topic_id'];
            ...
            if ($config['view']['actor_detail'] == 2) {
                ...
                switch ($config['rewrite']['actor_id']) {
                    case 1:
                        ...
                        break;
                    case 2:
                        $id = mac_alphaID($info['actor_id'], false, $config['rewrite']['encode_len'],
                            $config['rewrite']['encode_key']);
                        break;
                    default:
                        $id = $info['actor_id'];
            ...
            if ($config['view']['role_detail'] == 2) {
                ...
                switch ($config['rewrite']['role_id']) {
                    case 1:
                        ...
                        break;
                    case 2:
                        $id = mac_alphaID($info['role_id'], false, $config['rewrite']['encode_len'],
                            $config['rewrite']['encode_key']);
                        break;
                    default:
                        $id = $info['role_id'];
            ...
            if ($config['view']['website_detail'] == 2) {
                ...
                switch ($config['rewrite']['website_id']) {
                    case 1:
                        ...
                        break;
                    case 2:
                        $id = mac_alphaID($info['website_id'], false, $config['rewrite']['encode_len'],
                            $config['rewrite']['encode_key']);
                        break;
                    default:
                        $id = $info['website_id'];
...
function mac_label_website_detail($param)
{
    ...
    if ($GLOBALS['config']['rewrite']['website_id'] == 1) {
        ...
        if ($GLOBALS['config']['rewrite']['website_id'] == 2) {
            $param['id'] = mac_alphaID($param['id'], true, $GLOBALS['config']['rewrite']['encode_len'],
                $GLOBALS['config']['rewrite']['encode_key']);
        }
...
{
    $rex = ['{:', '<script', '<iframe', '<frameset', '<object', 'onerror'];
    if (is_array($rs)) {
        foreach ($rs as $k2 => $v2) {
            if (!is_numeric($v2)) {
...
{
    $psm = [
        'wd', 'tag', 'class', 'letter', 'name', 'state', 'level', 'area', 'lang', 'version', 'actor', 'director',
        'starsign', 'blood'
    ];
    foreach ($psm as $v) {
