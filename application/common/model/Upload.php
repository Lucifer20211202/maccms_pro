...
class Upload extends Base
{
    ...
    public function upload($p = [])
    {
        ...
        if (!empty($base64_img)) {
            if (preg_match('/^(data:\s*image\/(\w+);base64,)/', $base64_img, $result)) {
                ...
                if (in_array($type, explode(',', $upload_image_ext))) {
                    if (!file_put_contents($_save_path.$_save_name,
        ...
        fclose($resource);
        if (preg_match("/(3c25.*?28.*?29.*?253e)|(3c3f.*?28.*?29.*?3f3e)|(3C534352495054)|(2F5343524950543E)|(3C736372697074)|(2F7363726970743E)/is",
            $hexCode)) {
