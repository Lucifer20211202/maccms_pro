...
class Image extends Base
{
    ...
    public function watermark($file_path, $config, $flag = 'vod')
    {
        ...
        try {
            ...
            $image->text($config['watermark_content']."", $config['watermark_font'], $config['watermark_size'],
                $config['watermark_color'], $config['watermark_location'])->save('./'.$file_path);
        } catch (\Exception $e) {
        ...
        if (!empty($config['thumb_size'])) {
            try {
                ...
                foreach ($thumbs as $k => $v) {
                    ...
                    $new_thumb = $file_path.'_'.$t_size[0].'x'.$t_size[1].'.'.strtolower(pathinfo($file_path,
                            PATHINFO_EXTENSION));
                    if ($new == 0) {
                        $new_thumb = $file_path;
                    }
                    ...
                    if ($config['watermark'] == 1) {// 开启文字水印
                        ...
                        $image->text($config['watermark_content'], $config['watermark_font'], $config['watermark_size'],
                            $config['watermark_color'])->save('./'.$new_thumb);
                    }
                }
            } catch (\Exception $e) {
