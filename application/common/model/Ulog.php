...
class Ulog extends Base
{
        ...
        foreach ($list as $k => &$v) {
            ...
            if ($v['ulog_mid'] == 1) {
                ...
                if ($v['ulog_sid'] > 0 && $v['ulog_nid'] > 0) {
                    if ($v['ulog_type'] == 5) {
                        $vod_info['info']['link'] = mac_url_vod_down($vod_info['info'],
                            ['sid' => $v['ulog_sid'], 'nid' => $v['ulog_nid']]);
                    } else {
                        $vod_info['info']['link'] = mac_url_vod_play($vod_info['info'],
                            ['sid' => $v['ulog_sid'], 'nid' => $v['ulog_nid']]);
                    }
                } else {
    ...
    public function saveData($data)
    {
        ...
        if ($data['user_id'] == 0 || !in_array($data['ulog_mid'], ['1', '2', '3', '8']) || !in_array($data['ulog_type'],
                ['1', '2', '3', '4', '5'])) {
    ...
    public function createTableIfNotExists()
    {
        ...
        // 1
        $this->checkAndAlterTableField('ulog_watch_progress',
            "ADD `ulog_watch_progress` tinyint(3) UNSIGNED NOT NULL DEFAULT 0");
    }
