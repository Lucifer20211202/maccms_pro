...
class User extends Base
{
    ...
    public function login($param)
    {
        ...
        cookie('user_check', md5($random.'-'.$row['user_name'].'-'.$row['user_id'].'-'.request()->ip()),
            ['expire' => 2592000]);
    ...
    public function popedom($type_id, $popedom, $group_id = 1)
    {
        ...
        if (strpos(','.$group_info['group_type'],
                ','.$type_id.',') !== false && !empty($group_info['group_popedom'][$type_id][$popedom]) !== false) {
            return true;
        }
        return false;
    }
    ...
    public function check_msg($param)
    {
        ...
        if (!in_array($param['ac'],
                ['email', 'phone']) || empty($param['to']) || empty($param['code']) || empty($param['type'])) {
    ...
    public function send_msg($param)
    {
        ...
        if (!in_array($param['ac'], ['email', 'phone']) || !in_array($param['type'],
                ['1', '2', '3']) || empty($param['to']) || empty($param['type'])) {
            return ['code' => 9001, 'msg' => lang('param_err')];
        }
        ...
        if ($param['ac'] == 'email') {
            ...
            $msg = str_replace(['[用户]', '[类型]', '[时长]', '[验证码]'],
                [$GLOBALS['user']['user_name'], $type_des, '5', $code], $msg);
    ...
    public function visit($param)
    {
        ...
        $res = $this->where('user_id', $param['uid'])->setInc('user_points',
            intval($GLOBALS['config']['user']['invite_visit_points']));
        if ($res) {
            //积分日志
            $data = [];
            $data['user_id'] = $param['uid'];
            ...
            if (!empty($GLOBALS['config']['user']['reward_ratio']) && !empty($GLOBALS['user']['user_pid'])) {
                ...
                if ($points > 0) {
                    ...
                    if ($r) {
                        ...
                        $data['plog_remarks'] = lang('model/user/reward_tip',
                            [$GLOBALS['user']['user_id'], $GLOBALS['user']['user_name'], $fee_points, $points]);
                        model('Plog')->saveData($data);
                    }
                }
            }
