...
class Collection
{
        ...
        if ($html = self::get_html($url, $config)) {
            ...
            if (empty($page)) {
                ...
                if ($config['title_rule']) {
                    if (strpos($config['title_rule'], '[内容]') === false) {
                        ...
                        $data['title'] = self::replace_item(self::cut_html($html, $title_rule[0], $title_rule[1]),
                            $config['title_html_rule']);
                    }
                }
                ...
                if ($config['type_rule']) {
                    if (strpos($config['type_rule'], '[内容]') === false) {
                        ...
                        $data['type'] = self::replace_item(self::cut_html($html, $type_rule[0], $type_rule[1]),
                            $config['type_html_rule']);
                    }
                }
                ...
                if ($config['customize_config']) {
                    foreach ($config['customize_config'] as $k => $v) {
                        ...
                        if (strpos($v['rule'], '[内容]') === false) {
                            ...
                            $data[$v['en_name']] = self::replace_item(self::cut_html($html, $rule[0], $rule[1]),
                                $v['html_rule']);
                        }
                    }
                }
            ...
            if ($config['content_rule']) {
                if (strpos($config['content_rule'], '[内容]') === false) {
                    ...
                    $data['content'] = self::replace_item(self::cut_html($html, $content_rule[0], $content_rule[1]),
                        $config['content_html_rule']);
                }
            ...
            //处理分页
            if (in_array($page,
                    [0, 2]) && !empty($config['content_page_start']) && !empty($config['content_page_end'])) {
                $oldurl[] = $url;
                $tmp[] = $data['content'];
                $page_html = self::cut_html($html, $config['content_page_start'], $config['content_page_end']);
                //上下页模式
                if ($config['content_page_rule'] == 2 && in_array($page, [0, 2]) && $page_html) {
            ...
            if ($page == 0) {
                self::$url = $url;
                self::$config = $config;
                $data['content'] = preg_replace_callback('/<img[^>]*src=[\'"]?([^>\'"\s]*)[\'"]?[^>]*>/i',
                    ['collection', 'download_img_callback'], $data['content']);
                //下载内容中的图片到本地
    ...
    protected static function url_check($url, $baseurl, $config)
    {
        ...
        $baseurl = $urlinfo['scheme'].'://'.$urlinfo['host'].(substr($urlinfo['path'], -1,
                1) === '/' ? substr($urlinfo['path'], 0, -1) : str_replace('\\', '/', dirname($urlinfo['path']))).'/';
        if (strpos($url, '://') === false) {
            if ($url[0] == '/') {
