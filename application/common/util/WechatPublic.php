...
class WechatPublic
{
    ...
    private function receiveText($object)
    {
        ...
        if ($this->_conf['gjc1'] <> '' && strstr($content, $this->_conf['gjc1'])) {
            ...
            $data[] = [
                'Title' => $this->_conf['gjcm1'], 'Description' => '', 'PicUrl' => $this->_conf['gjci1'],
                'Url'   => $this->_conf['gjcl1']
            ];
        } elseif ($this->_conf['gjc2'] <> '' && strstr($content, $this->_conf['gjc2'])) {
            $data = [];
            $txt .= '<a href="'.$this->_conf['gjcl2'].'">'.$this->_conf['gjcm2'].'</a>'."\n";
            $data[] = [
                'Title' => $this->_conf['gjcm2'], 'Description' => '', 'PicUrl' => $this->_conf['gjci2'],
                'Url'   => $this->_conf['gjcl2']
            ];
        } elseif ($this->_conf['gjc3'] <> '' && strstr($content, $this->_conf['gjc3'])) {
            $data = [];
            $txt .= '<a href="'.$this->_conf['gjcl3'].'">'.$this->_conf['gjcm3'].'</a>'."\n";
            $data[] = [
                'Title' => $this->_conf['gjcm3'], 'Description' => '', 'PicUrl' => $this->_conf['gjci3'],
                'Url'   => $this->_conf['gjcl3']
            ];
        } elseif ($this->_conf['gjc4'] <> '' && strstr($content, $this->_conf['gjc4'])) {
            $data = [];
            $txt .= '<a href="'.$this->_conf['gjcl4'].'">'.$this->_conf['gjcm4'].'</a>'."\n";
            $data[] = [
                'Title' => $this->_conf['gjcm4'], 'Description' => '', 'PicUrl' => $this->_conf['gjci4'],
                'Url'   => $this->_conf['gjcl4']
            ];
        } else {
            $param = [];
            $param['wd'] = $content;
            ...
            if ($res['code'] > 1 || empty($res['list'])) {
                ...
                $data[] = [
                    'Title' => $this->_conf['wuziyuan'], 'Description' => '', 'PicUrl' => '',
                    'Url'   => $this->_conf['wuziyuanlink']
                ];
            } else {

                if ($this->_conf['bofang'] == 2) {
                    $search_url = $this->_conf['sousuo'].mac_url('vod/search',
                            ['wd' => $content]); //'/index.php/vod/search/wd/' . urlencode($content);
                    $txt .= '<a href="'.$search_url.'">点击进入搜索页面查看</a>'."\n";
                    $data[] = [
                        'Title'       => '点击进入搜索页面查看',
                        'Description' => '恭喜您找到了相关资源，由于微信限制请进入搜索页查看', 'PicUrl' => '',
                        'Url'         => $search_url
                    ];
                } else {
                    foreach ($res['list'] as $k => $v) {
                        ...
                        $data[] = [
                            'Title'       => $v['vod_name'],
                            'Description' => mac_substring(strip_tags($v["vod_content"]), 20), 'PicUrl' => $picUrl,
                            'Url'         => $url
                        ];
                    }
                }
