...
class Annex extends Base
{
                ...
                foreach ($farr as $f) {
                    ...
                    if (is_dir($f)) {
                        ...
                        if (strpos($f, ".html") <= 0 && strpos($f, ".htm") <= 0) {
                            ...
                            $files[] = [
                                'isfile' => 1, 'name' => $tmp_name, 'path' => $tmp_path, 'size' => $fsize,
                                'time'   => $ftime
                            ];
                        }
                    }
    ...
    public function check()
    {
        ...
        mac_echo(lang('admin/annex/info_tip',
            [$param['data_count'], $param['page_count'], $param['page_size'], $start]));
    ...
    public function init()
    {
        ...
        if ($param['ck']) {
            ...
            $schema = Db::query('select * from information_schema.columns where table_schema = ?',
            ...
            foreach ($col_list as $k1 => $v1) {
                ...
                foreach ($list as $k3 => $v3) {
                    ...
                    if (substr($val, 0, 6) == 'upload' && file_exists('./'.$val)) {
                        $imgs[] = [
                            'annex_file' => $val, 'annex_time' => time(), 'annex_size' => filesize('./'.$val),
                            'annex_type' => 'image'
                        ];
                        $ck = true;
                    }
                    ...
                    $val = $v3[$col];
                    if (!empty($val)) {
                        $rule = mac_buildregx("<img[^>]*src\s*=\s*['".chr(34)."]?([\w/\-\:.]*)['".chr(34)."]?[^>]*>",
                            "is");
                        ...
                        foreach ($matchfieldarr as $f => $matchfieldstr) {
                            ...
                            if (substr($img_src, 0, 7) == '/upload' && file_exists('.'.$img_src)) {
                                $imgs[] = [
                                    'annex_file' => substr($img_src, 1), 'annex_time' => time(),
                                    'annex_size' => filesize('.'.$img_src), 'annex_type' => 'image'
                                ];
                                $ck = true;
                            }
                        }
