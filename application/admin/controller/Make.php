...
class Make extends Base
{
    ...
    public function type()
    {
        ...
        $this->echoLink(lang('admin/make/type_tip',
            [$type_info['type_name'], $this->_param['page_count'], $sec_count, $sec]));
        $this->label_maccms();
    ...
    public function info()
    {
        ...
        if (!empty($type_ids)) {
            ...
            if ($num >= count($type_ids)) {
                ...
                if (empty($this->_param['jump'])) {
                    ...
                    if (ENTRANCE == 'admin') {
                        mac_jump(url('make/make', [
                            'jump' => 1, 'ac' => 'type', 'tab' => $this->_param['tab'],
                            $this->_param['tab'].'type' => join(',', $type_ids), 'ac2' => 'day'
                        ]), 3);
                    }
                    exit;
                }
        ...
        $this->echoLink(lang('admin/make/info_tip', [
            $type_name, $this->_param['data_count'], $this->_param['page_count'], $this->_param['page_size'], $start
        ]));
        ...
        foreach ($res['list'] as $k => $v) {
            ...
            if (!empty($v['art_id'])) {
                ...
                if (!empty($v['vod_play_from'])) {
                    $v['vod_play_list'] = mac_play_list($v['vod_play_from'], $v['vod_play_url'], $v['vod_play_server'],
                        $v['vod_play_note'], 'play');
                    $v['vod_play_total'] = count($v['vod_play_list']);
                }
                if (!empty($v['vod_down_from'])) {
                    $v['vod_down_list'] = mac_play_list($v['vod_down_from'], $v['vod_down_url'], $v['vod_down_server'],
                        $v['vod_down_note'], 'down');
                ...
                foreach ($flag as $f) {
                    ...
                    if ($GLOBALS['config']['view']['vod_'.$f] < 2) {
                        ...
                        if ($GLOBALS['config']['view']['vod_'.$f] == 2) {
                            ...
                            $link = ($f == 'play' ? mac_url_vod_play($v,
                                ['sid' => 1, 'nid' => 1]) : mac_url_vod_down($v, ['sid' => 1, 'nid' => 1]));
                            ...
                            for ($i = 1; $i <= $v['vod_'.$f.'_total']; $i++) {
                                for ($j = 1; $j <= $v['vod_'.$f.'_list'][$i]['url_count']; $j++) {
                                    ...
                                    $info = $this->label_vod_play($f, $v, $GLOBALS['config']['view']['vod_'.$f]);
                                    $link = ($f == 'play' ? mac_url_vod_play($v,
                                        ['sid' => $i, 'nid' => $j]) : mac_url_vod_down($v, ['sid' => $i, 'nid' => $j]));
                            ...
                            for ($i = 1; $i <= $v['vod_'.$f.'_total']; $i++) {
                                ...
                                $info = $this->label_vod_play($f, $v, $GLOBALS['config']['view']['vod_'.$f]);
                                $link = ($f == 'play' ? mac_url_vod_play($v, ['sid' => $i]) : mac_url_vod_down($v,
                                    ['sid' => $i]));
                                $link_sp = explode('?', $link);
                                $this->buildHtml($link_sp[0], './', mac_tpl_fetch('vod', $info['vod_tpl_'.$f], $f));
                                if ($i == 1) {
        ...
        if (!empty($update_ids)) {
            Db::name($this->_param['tab'])->where([
                $this->_param['tab'].'_id' => [
                    'in', $update_ids
                ]
            ])->update([$this->_param['tab'].'_time_make' => time()]);
        }
        if ($this->_param['ref'] == 1 && !empty($_SERVER["HTTP_REFERER"])) {
            if (ENTRANCE == 'admin') {
                mac_jump($_SERVER["HTTP_REFERER"], 2);
