...
class Website extends Base
{
    ...
    public function batch()
    {
        ...
        if (!empty($param)) {
            ...
            mac_echo("<font color=red>".lang('admin/batch_tip',
                    [$param['total'], $param['limit'], $param['page_count'], $param['page']])."</font>");
    ...
    public function info()
    {
        if (Request()->isPost()) {
            $param = input('post.');
            $param['website_content'] = str_replace($GLOBALS['config']['upload']['protocol'].':', 'mac:',
    ...
    public function field()
    {
        ...
        if (!empty($ids) && in_array($col,
                ['website_status', 'website_lock', 'website_level', 'website_hits', 'type_id'])) {
            $where = [];
            $where['website_id'] = ['in', $ids];
            $update = [];
            if (empty($start)) {
