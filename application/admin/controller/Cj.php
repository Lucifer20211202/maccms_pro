...
class Cj extends Base
{
            ...
            if (!empty($data['customize_config'])) {
                ...
                foreach ($customize_config['name'] as $k => $v) {
                    ...
                    $data['customize_config'][] = [
                        'name' => $customize_config['name'][$k], 'en_name' => $customize_config['en_name'][$k],
                        'rule' => $customize_config['rule'][$k], 'html_rule' => $customize_config['html_rule'][$k]
                    ];
                }
    ...
    public function program()
    {
        ...
        $node_field = ['title' => lang('title'), 'type' => lang('type'), 'content' => lang('content')];
        $customize_config = [];
        if (!empty($res['info']['customize_config'])) {
        ...
        if (is_array($customize_config)) {
            foreach ($customize_config as $k => $v) {
    ...
    public function col_url($param = [])
    {
        ...
        if (is_array($url) && !empty($url)) {
            foreach ($url as $v) {
                ...
                if ($history['code'] > 1) {
                    Db::name('cj_history')->insert(['md5' => $md5]);
                    Db::name('cj_content')->insert([
                        'nodeid' => $param['id'], 'status' => 1, 'url' => $v['url'], 'title' => $v['title']
                    ]);
                } else {
                    $re++;
                }
        ...
        if ($total_page <= $param['page']) {
            $time = time();
            Db::name('cj_node')->where('nodeid', $param['id'])->update(['lastdate' => $time]);
        }
        if ($this->_isall == 1) {
    ...
    public function col_content($param = [])
    {
        ...
        $list = Db::name('cj_content')->where('nodeid', $param['id'])->where('status', 1)->page($total_page - 1,
            $limit)->select();
