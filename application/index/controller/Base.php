...
class Base extends All
{
    ...
    protected function check_user_popedom($type_id, $popedom, $param = [], $flag = '', $info = [], $trysee = 0)
    {
        ...
        $res = false;
        if (strpos(','.$group['group_type'],
                ','.$type_id.',') !== false && !empty($group['group_popedom'][$type_id][$popedom]) !== false) {
            $res = true;
        }
        $pre = $flag;
        $col = 'detail';
        ...
        if ($GLOBALS['config']['user']['status'] == 0) {
            ...
            if ($res === false && (empty($group['group_popedom'][$type_id][2]) || $trysee == 0)) {
                ...
                if ($res['code'] > 1) {
                    return ['code'   => 3003, 'msg' => lang_frontend('controller/pay_play_points', [$points]),
                            'points' => $points, 'confirm' => 1, 'trysee' => 0
                    ];
                }
            }
        } elseif ($popedom == 3) {
            ...
            if ($popedom == 4) {
                if ($group['group_id'] == 1 && $points > 0) {
                    ...
                    if ($res['code'] > 1) {
                        return ['code'   => 4003, 'msg' => lang_frontend('controller/pay_down_points', [$points]),
                                'points' => $points, 'confirm' => 1, 'trysee' => 0
                        ];
                    }
                }
                ...
                if (empty($group['group_popedom'][$type_id][3]) && !empty($group['group_popedom'][$type_id][5])) {
                    ...
                    if (2 == 1) {
                        ...
                        return ['code'   => 5001,
                                'msg'    => lang_frontend('controller/try_see_end', [$points, $user['user_points']]),
                                'trysee' => $trysee
                        ];
                    } elseif ($group['group_id'] < 3 && $points > intval($user['user_points'])) {
                        return ['code' => 5002, 'msg' => lang_frontend('controller/not_enough_points',
                            [$points, $user['user_points']]), 'trysee' => $trysee
                        ];
                    }
                }
            }
